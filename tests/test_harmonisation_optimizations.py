...
def _cleanup_layout():
    return {
        ...
        "chromosome_table": "{dataset_id}_chr{chromosome}.parquet",
        "chromosome_source_snapshot": (
            "rejected/{dataset_id}_chr{chromosome}_source.parquet"
        ),
        "external_eaf_partition": (
            "qc_summary/.external_reference/{dataset_id}_eaf_chr{chromosome}.parquet"
        ),
        "external_info_partition": (
            "qc_summary/.external_reference/{dataset_id}_info_chr{chromosome}.parquet"
        ),
    }
...
def _write_adapter_summary(
    ...
    pl.DataFrame([record]).select(
        list(GWAS2VCF_SUMMARY_COLUMNS)
    ).write_csv(path, separator=delimiter)


def test_shared_variant_canonicalization_applies_one_policy_to_all_fields():
    warnings = []
    frame = pl.DataFrame({
        "CHR": ["chr01", "23"],
        "POS": ["100.0", "not-a-position"],
        "EA": [" a ", ""],
        "OA": [" g ", " t "],
    })

    normalized, counts = canonicalize_variant_frame(
        frame,
    ...
    assert normalized["CHR"].to_list() == ["1", "X"]
    assert normalized["POS"].to_list() == [100, None]
    assert normalized["EA"].to_list() == ["A", None]
    assert normalized["OA"].to_list() == ["G", "T"]
    assert counts == {
        "positions_present": 2,
        "positions_retained": 1,
        "positions_unreadable": 1,
...
def test_shared_dna_orientation_primitives_reject_non_dna_and_multibase_palindromes():
    frame = pl.DataFrame({
        "EA": [" a ", "AT", "N", "C"],
        "OA": ["t", "TA", "A", "G"],
    }).select([
        reverse_complement_expression(pl.col("EA")).alias("RC"),
        palindromic_snp_expression(
            pl.col("EA"), pl.col("OA")
        ).alias("PALINDROMIC_SNP"),
    ])

    assert frame["RC"].to_list() == ["T", "AT", None, "G"]
    assert frame["PALINDROMIC_SNP"].to_list() == [True, False, False, True]
...
def test_harmonisation_service_public_exports_exist():
    from postgwas.modules.harmonisation import service

    assert [name for name in service.__all__ if not hasattr(service, name)] == []


@pytest.mark.parametrize(
    "suffix",
    [".tsv", ".tsv.gz", ".tsv.bgz", ".tsv.bz2", ".tsv.xz", ".zip"],
)
def test_chunked_input_inspection_preserves_compression_and_line_semantics(
    tmp_path,
    suffix,
):
    source = tmp_path / ("study" + suffix)
    content = b"##metadata\r\nCHR\tPOS\r1\t10\n##later\n1\t20"
    if suffix in (".tsv.gz", ".tsv.bgz"):
        ...
        with zipfile.ZipFile(source, "w", compression=zipfile.ZIP_DEFLATED) as archive:
            archive.writestr("study.tsv", content)
    else:
...
def test_chunked_input_inspection_preserves_final_line_warning(tmp_path):
    source = tmp_path / "study.tsv"
    source.write_bytes(b"CHR\tPOS\r\n1\t10.\xc2\xa0")
    ...
    assert data_rows == 1
    assert warnings == [
        "The last line ends with '.', so the file may be truncated"
    ]


def test_chunked_input_inspection_reports_truncated_gzip(tmp_path):
    source = tmp_path / "study.tsv.gz"
    with gzip.open(source, "wb") as handle:
        handle.write(b"CHR\tPOS\n1\t10\n")
    payload = source.read_bytes()
    source.write_bytes(payload[:-4])
    ...
    assert data_rows is None
    assert len(warnings) == 1
    assert warnings[0].startswith("The file could not be read completely:")
...
def test_chromosome_partition_preserves_rows_and_columns(tmp_path):
    ...
    partitions = write_chromosome_partitions(
        frame,
        {"output_folder": str(tmp_path), "chr_col": "CHR", "gwas_outputname": "study"},
        {"chromosome_table": "{dataset_id}_chr{chromosome}.parquet"},
        default_policies().get("input.chromosome_partition_compression"),
    )
    paths = partitions.paths

    assert set(paths) == {"1", "2", "X"}
    assert partitions.rows == {"2": 2, "1": 1, "X": 1}
    chromosome_two = pl.read_parquet(paths["2"])
    assert chromosome_two.columns == frame.columns
    assert chromosome_two["POS"].to_list() == [20, 21]
    assert chromosome_two.schema == frame.schema
    assert sum(pl.read_parquet(path).height for path in paths.values()) == frame.height


def test_chromosome_partition_does_not_materialise_all_partitions(monkeypatch, tmp_path):
    frame = pl.DataFrame({
        "CHR": ["1", "2", "1", "X"],
        "POS": [10, 20, 11, 30],
        "EA": ["A", "C", "G", "T"],
    ...
    def fail_partition_by(*args, **kwargs):
        raise AssertionError("partition_by must not materialise every chromosome")

    monkeypatch.setattr(pl.DataFrame, "partition_by", fail_partition_by)
    partitions = write_chromosome_partitions(
        frame,
        {
    ...
    assert list(paths) == ["1", "2", "X"]
    assert partitions.rows == {"1": 2, "2": 1, "X": 1}
    assert pl.read_parquet(paths["1"])["POS"].to_list() == [10, 11]
...
def test_chromosome_partition_refuses_incomplete_source_provenance(tmp_path):
    source = pl.DataFrame({
        SOURCE_INPUT_ROW_COLUMN: [1],
        "CHR": ["1"],
        "POS": [100],
    })
    working = pl.DataFrame({
        SOURCE_INPUT_ROW_COLUMN: [1, 2],
        "CHR": ["1", "1"],
        "POS": [100, 200],
    })
    layout = {
    ...
    with pytest.raises(ValueError, match="2 working rows but 1 matching"):
        write_chromosome_partitions(
            working,
            {
                "output_folder": str(tmp_path),
    ...
    assert not (tmp_path / "study_chr1.parquet").exists()
    assert not (tmp_path / "rejected" / "study_chr1_source.parquet").exists()


def test_successful_cleanup_removes_internal_source_snapshots(tmp_path):
    chromosome_table = tmp_path / "study_chr1.parquet"
    ...
    external_partition = (
        tmp_path / "qc_summary" / ".external_reference"
        / "study_eaf_chr1.parquet"
    )
    source_snapshot.parent.mkdir()
    external_partition.parent.mkdir(parents=True)
    pl.DataFrame({"CHR": ["1"], "POS": [100]}).write_parquet(chromosome_table)
    pl.DataFrame({"CHR": ["1"], "POS": [100]}).write_parquet(source_snapshot)
    pl.DataFrame({"CHROM": ["1"], "POS": [100]}).write_parquet(
        external_partition
    )
    adapter_mapping.write_text('{"chr_col": 0}\n', encoding="utf-8")
    _write_adapter_summary(adapter_summary, "1", 1)
    ...
    result = finalise_harmonisation_outputs(
        ...
        expected_rows_by_chromosome={"1": 1},
        threads=1,
...
def test_cleanup_rejects_inconsistent_chromosome_column_mappings(tmp_path):
    ...
    for chromosome in ("1", "2"):
        _write_adapter_summary(
            tmp_path / ("study_chr%s_summary.tsv" % chromosome),
            chromosome,
            1,
        )
    ...
    with pytest.raises(RuntimeError, match="column mappings are inconsistent"):
        finalise_harmonisation_outputs(
            ...
            expected_rows_by_chromosome={"1": 1, "2": 1},
            threads=1,
            compression_executable=None,
        )
...
def test_study_variant_columns_are_canonicalised_once_and_partition_types_restore(tmp_path):
    ...
    source.write_text(
        ...
        "chr01\t100.0\ta\tg\t000123\t0.25\t1e-400\n",
        encoding="utf-8",
    )
    (
        frame,
        ...
        _removed_unsupported_chromosomes,
        _removed_alleles,
        _removed_duplicates,
        _removed_missing,
        _field_completeness,
    ...
    assert frame.schema["P"] == pl.Float64
    assert frame.item(0, "P") == 0.0
    source_pvalue_column = mapping[P_VALUE_SOURCE_TEXT_KEY]
    assert frame.schema[source_pvalue_column] == pl.String
    assert frame.item(0, source_pvalue_column) == "1e-400"
    ...
    paths = partitions.paths
    restored = pl.read_parquet(paths["1"])
    ...
    assert restored.schema[source_pvalue_column] == pl.String
    assert restored.item(0, source_pvalue_column) == "1e-400"
...
def test_direct_content_validation_reuses_canonical_chromosome_policy():
    frame = pl.DataFrame({
        "CHR": ["01", "23"],
        "POS": [100, 200],
        "EA": ["A", "C"],
...
def test_partition_schema_keeps_snp_and_combined_coordinate_as_strings():
    mapping = {
        ...
        "pvalue_source_text_col": "__pval_source_text",
    }
...
def test_dot_is_a_default_missing_token_and_required_p_is_rejected(tmp_path):
    ...
    assert completeness["rows_removed_at_read_mandatory_gate"] == 1
...
def test_separate_fractional_position_is_rejected_instead_of_truncated():
    frame, _mapping, qc = harmonise_coordinates_and_alleles(
        chromosome="All_Chrs",
        df=pl.DataFrame({
    ...
    assert qc["positions_non_integral"] == 1
    assert qc["removed_by_reason"]["invalid_position"] == 1


def test_separate_integer_valued_position_representations_are_preserved():
    frame, _mapping = harmonise_coordinates_and_alleles(
        chromosome="All_Chrs",
        df=pl.DataFrame({
            "ID": ["integer", "decimal_integer", "scientific_integer"],
            "CHR": ["1", "1", "1"],
            "POS": ["100000", "100000.0", "1e5"],
            "EA": ["A", "A", "A"],
            "OA": ["G", "G", "G"],
        }),
        sample_column_dict={
    ...
    assert frame["ID"].to_list() == [
        "integer", "decimal_integer", "scientific_integer",
    ]
    assert frame["POS"].to_list() == [100000, 100000, 100000]


def test_combined_fractional_position_is_rejected_after_numeric_token_extraction():
    frame, _mapping, qc = harmonise_coordinates_and_alleles(
        chromosome="All_Chrs",
        df=pl.DataFrame({"VARIANT": ["1:100000.9:A:G"]}),
        sample_column_dict={
            "chr_col": None,
            "pos_col": None,
            "chr_pos_col": "VARIANT",
...
def test_info_aggregate_counts_match_configured_actions():
    ...
    assert qc["lt_zero"] == 1
    assert qc["gt_one"] == 2
    assert qc["gt_standard_info_tolerance"] == 1
    assert qc["rescaled_within_tolerance"] == 1
    assert qc["out_of_range"] == 2
    assert qc["missing_info_after"] == 3
    assert result["INFO"].to_list() == [None, None, 0.5, 1.0, None]


def test_info_rejection_keeps_missing_values_for_the_missing_value_policy():
    policies = default_policies().with_overrides({
        "info": {
            "source": "study",
            "score_type": "standard_info",
    ...
    assert result["INFO"].to_list() == [None, 0.5]
    assert qc["rejected_out_of_range"] == 2
    assert qc["missing_info_after"] == 1
    assert qc["rejected_missing"] == 0
...
def test_chromosomewise_metrics_preserve_round_trippable_float64_precision(
    tmp_path,
):
    path = tmp_path / "study_chromosomewise_harmonisation_metrics.tsv"
    ...
    _save_qc_results(
        {"1": {"effect_qc": {"precise_metric": expected}}},
        path,
        allowed_chromosomes=["1"],
        delimiter="\t",
        null_output="",
    )
    ...
    assert float(row["chr1"]) == expected
