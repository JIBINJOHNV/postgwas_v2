...
class Vcf:
    ...
    def write_to_file(
        ...
        header.add_line(
            '##FORMAT=<ID=NCO,Number=A,Type=Integer,Description="Number of control used to estimate genetic effect">'
        )
        header.add_line(
            '##FORMAT=<ID=NEF,Number=A,Type=Integer,Description="Number of effective sample size, in case of case-control  Nef=4/(1/ncase + 1/ncontrol); for quantitative Nef=ncontrol">'
        )
        header.add_line(
        ...
        for contig in fasta.references:
            ...
            while gwas_idx[contig]:
                ...
                if result.ncontrol is not None:
                    record.samples[trait_id]["NCO"] = round(result.ncontrol)
                if result.neff is not None:
                    record.samples[trait_id]["NEF"] = round(result.neff)
                if study_id is not None:
                    record.samples[trait_id]["ID"] = study_id
                # write to file
                vcf.write(record)
