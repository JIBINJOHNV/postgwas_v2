...
import gzip  # Added for reading gzipped input files
...
def main():
    ...
    with open(args.data, 'r') if not args.data.endswith('.gz') else gzip.open(args.data, 'rt') as f:
        header = f.readline().strip().split(json_data["delimiter"])
        num_columns = len(header)
        for col_name, col_idx in [
            ("chr_col", json_data["chr_col"]),
            ("pos_col", json_data["pos_col"]),
            ("ea_col", json_data["ea_col"]),
            ("oa_col", json_data["oa_col"]),
            ("beta_col", json_data["beta_col"]),
            ("se_col", json_data["se_col"]),
            ("pval_col", json_data["pval_col"]),
            ("snp_col", json_data.get("snp_col")),
            ("eaf_col", json_data.get("eaf_col")),
            ("oaf_col", json_data.get("oaf_col")),
            ("imp_z_col", json_data.get("imp_z_col")),
            ("imp_info_col", json_data.get("imp_info_col")),
    ...
    with pysam.FastaFile(args.ref) as fasta:
        gwas, idx, sample_metadata = Gwas.read_from_file(
            ...
            study_id_col_num=json_data.get("snp_col"),
