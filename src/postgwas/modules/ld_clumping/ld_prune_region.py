...
def ld_clump_by_regions(
    ...
    # Define filenames
    raw_out = output_directory / f"{dataset_id}_vcf.tsv.gz"
    pruned_out = output_directory / f"{dataset_id}_LDpruned_{population}.tsv"
    pruned_sig_out = output_directory / f"{dataset_id}_LDpruned_{population}_sig.tsv"
    log_file = output_directory / f"{dataset_id}_ld_clump.log"
    ...
    set -o pipefail
    {{
        printf "SNP\\tCHR\\tBP\\tREF\\tALT\\tBETA\\tSE\\tNEF\\tAF\\tAFR\\tEAS\\tEUR\\tSAS\\tLP\\tNC\\tNCO\\t{ld_field}\\tBCSQ\\n"
        {bcftools} view --threads {threads} --min-alleles 2 --max-alleles 2 "{vcf_path}" | \
        {bcftools} query -f '%CHROM:%POS:%REF:%ALT\\t%CHROM\\t%POS\\t%REF\\t%ALT\\t[%ES]\\t[%SE]\\t[%NEF]\\t[%AF]\\t[%AFR]\\t[%EAS]\\t[%EUR]\\t[%SAS]\\t[%LP]\\t[%NC]\\t[%NCO]\\t%INFO/{ld_field}\\t[%BCSQ]\\n'
    }} | gzip -c > "{raw_out}"
    """
