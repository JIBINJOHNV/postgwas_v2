...
def _installed_script(name: str) -> str:
    """Locate an LDSC entry point installed in the active PostGWAS environment."""
    path = shutil.which(name)
    if path is None:
        raise FileNotFoundError(
            f"{name} is not installed in the active PostGWAS environment"
        )
    return path
...
def run_ldsc(
    ...
    LDSC runs with the same Python environment as PostGWAS.
    """
    ...
    try:
        ...
        weight_ldscore_dir = Path(weight_ldscore_dir).expanduser().resolve(strict=True)
        out_prefix = Path(out_prefix).expanduser().resolve()
        out_prefix.parent.mkdir(parents=True, exist_ok=True)
    except Exception as e:
        raise ValueError(f"LDSC input or reference path is invalid: {e}") from e

    munge_script = _installed_script("munge_sumstats.py")
    ldsc_script = _installed_script("ldsc.py")
    ...
    if not Path(munged_file).exists():
        raise RuntimeError(f"LDSC munging completed without creating {munged_file}")
    ...
    if samp_prev is not None and pop_prev is not None:
        ...
        cmd_liability = [
            sys.executable, ldsc_script,
            "--h2", _d(munged_file),
            "--ref-ld-chr", _d(ldscore_dir) + "/",
            "--w-ld-chr", _d(weight_ldscore_dir) + "/",
            "--out", str(out_liab),
