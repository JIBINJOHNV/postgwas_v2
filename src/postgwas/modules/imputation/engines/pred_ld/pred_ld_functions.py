...
# Ensure REF_DIR exists globally (defaults to "ref")
if not hasattr(builtins, "REF_DIR"):
    builtins.REF_DIR = "ref"

print(builtins.REF_DIR)
# harmonise funcitons
...
def Hap_Map_LD_info_dask(rs_list, chrom, population, maf_threshold, R2_threshold, imp_snp_list):
    ...
    ref_dir = builtins.REF_DIR
    out_dir = builtins.OUT_DIR
    ...
    if population not in ['YRI', 'CHB', 'JPT', 'CEU', 'MKK', 'LWK', 'CHD', 'GIH', "TSI", 'MEX', "ASW"]:
        ...
        exit()
    maf_file = f'{ref_dir}/Hap_Map/allele_freqs_chr{chrom}_{population}_phase3.2_nr.b36_fwd.txt.gz'
    ld_file = f'{ref_dir}/Hap_Map/ld_chr{chrom}_{population}.txt.gz'
    maf_df = dd.read_csv(maf_file, sep=r'\s+', blocksize=None)
    ...
    ld_df = dd.read_csv(ld_file, blocksize=None, sep=r'\s+', header=None)
    ...
    final_result.rename(columns={"MAF1": "ALT_AF1", "MAF2": "ALT_AF2"}).to_csv(
        f'{out_dir}/LD_info_Hap_Map_chr' + str(chrom) + '.txt', sep="\t", index=False
    )
...
def pheno_Scanner_LD_info_dask(rs_list, chrom, population, maf_threshold, R2_threshold, imp_snp_list):
    ...
    maf_file = f'{ref_dir}/Pheno_Scanner/1000G.txt'
    ld_file = f'{ref_dir}/Pheno_Scanner/1000G_{population}/1000G_{population}_chr{chrom}.txt.gz'
    ...
    # Process LD DataFrame using Dask
    ld_df = dd.read_csv(ld_file, sep=r'\s+', blocksize=None,
   ...
   # final_result.to_csv('LD_info_Pheno_Scanner_chr_' + str(chrom) + '.txt', sep="\t", index=False)
    final_result.rename(columns={"MAF1": "ALT_AF1", "MAF2": "ALT_AF2"}).to_csv(
        f'{out_dir}/LD_info_Pheno_Scanner_chr_' + str(chrom) + '.txt', sep="\t", index=False
    )
...
def TOP_LD_info(rs_list, chrom, population, maf_threshold, R2_threshold, imp_snp_list):
    ...
    maf_file = f'{ref_dir}/TOP_LD/' + population + '/SNV/'+ population + '_chr' + str(
        chrom) + '_no_filter_0.2_1000000_info_annotation.csv.gz'
    ld_file = f'{ref_dir}/TOP_LD/' + population + '/SNV/' + population + '_chr' + str(
    ...
    result.rename(columns={"MAF1": "ALT_AF1", "MAF2": "ALT_AF2"}).to_csv(
        f'{out_dir}/LD_info_TOP_LD_chr' + str(chrom) + '.txt', sep="\t", index=False
    )
    return result
...
def process_data(file_path, r2threshold, population, maf_input, ref_file, imp_snp_list):
    ...
    if ref_panel == 'TOP_LD':
        for chrom in chroms:
            ...
            final_data.to_csv(f"{out_dir}/imputation_results_chr" + str(chrom) + ".txt", sep="\t", index=False)
        ...
        if len(chroms) > 1:
            ...
            final_data.to_csv(f"{out_dir}/imputation_results_chr_all.txt", sep="\t", index=False)
