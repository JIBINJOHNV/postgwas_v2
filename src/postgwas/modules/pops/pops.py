...
def pops_main(config_dict, progress_callback=None):
    ### --------------------------------- Basic settings --------------------------------- ###
    ### --------------------------------- Logging --------------------------------- ###
    ...
    # Upstream diagnostics belong in its dedicated file. The PostGWAS service
    # renders ordered scientific progress from the optional parent callback.
    handlers = [
    ...
    if progress_callback is not None:
        progress_callback("target_loading", {
            "target_source": (
                "MAGMA gene Z statistics"
                if config_dict["magma_prefix"] is not None
                else "configured target-score table"
            ),
            "target_genes": len(Y_ids),
            "covariates": 0 if covariates is None else covariates.shape[1],
            "error_covariance": "used" if error_cov is not None else "not used",
        })
    ...
    if progress_callback is not None:
        progress_callback("covariate_adjustment", {
            "status": "applied" if covariates is not None else "not requested",
            "genes": int(project_out_covariates_Y_gene_inds.sum()),
            "covariates": 0 if covariates is None else covariates.shape[1],
            "hla_policy": (
                "excluded"
                if config_dict["project_out_covariates_remove_hla"]
                else "included"
            ),
        })
    ...
    if progress_callback is not None:
        progress_callback("feature_selection", {
            "strategy": (
                ...
                if config_dict["feature_selection_fss_num_features"] is not None
                else "marginal association p-value filtering"
            ),
            "genes": int(feature_selection_Y_gene_inds.sum()),
            "tested_features": len(marginal_assoc_df),
    ...
    selected_cv_alpha = None
    if "SELECTED_CV_ALPHA" in coefs_df.index:
        selected_cv_alpha = coefs_df.loc["SELECTED_CV_ALPHA", "beta"]
    if progress_callback is not None:
        progress_callback("model_fitting", {
            "method": config_dict["method"],
            "training_genes": X_train.shape[0],
            "model_features": X_train.shape[1],
            "selected_cv_alpha": selected_cv_alpha,
        })
    ### Prediction
    ...
    if progress_callback is not None:
        progress_callback("gene_scoring", {
            "genes_scored": len(preds_df),
            "model_features": len(cols),
            "outputs_written": 5 if config_dict["save_matrix_files"] else 3,
        })
