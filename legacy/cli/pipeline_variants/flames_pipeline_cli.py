...
Supports:
  ...
  • PIPELINE MODE:
        Full workflow:
            Harmonisation → LD Blocks → QC → Formatter →
            SuSiE → MAGMA → PoPS → FLAMES
...
def main():
    ...
    top = argparse.ArgumentParser(
        prog="flames",
        description=(
            "🔥 [bold cyan]FLAMES — Fine-mapped Locus Assessment Model of Effector geneS[/bold cyan]\n\n"
    ...
    pipeline = sub.add_parser(
        "pipeline",
        help="Run full PostGWAS → SuSiE → MAGMA → PoPS → FLAMES pipeline.",
        parents=[
            get_defaultresourse_parser(),
            get_inputvcf_parser(),
            get_genomeversion_parser(),
