...
## PostGWAS fine-mapping handoff

PostGWAS FLAMES consumes `downstream_inputs/flames/indexfile.txt` produced by
either the SuSiE or FINEMAP engine. It does not scan the interchange directory because
that directory also contains manifests and genomic-locus metadata that are not
credible-set tables. Before annotation starts, PostGWAS verifies the index
columns, every referenced file, the configured credible-set and probability
